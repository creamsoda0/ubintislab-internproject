	...
	function sendAuthNum() {
		...
		$.ajax({
			...
			success : function(result) {
				// 서버 응답이 200 OK인 경우에만 실행
				alert("인증번호가 이메일로 발송되었습니다.\n메일함을 확인해주세요.");
				$("#authnumber").focus();
			},
			error : function(xhr) {
				// 서버 응답이 4xx, 5xx 에러인 경우 실행
				// xhr.status에 따라 서버에서 보낸 상태 코드를 확인할 수 있습니다.

				if (xhr.status === 404) {
					// fail_no_user 대응 (HttpStatus.NOT_FOUND)
					alert("입력하신 정보와 일치하는 회원이 없습니다.");
				} else if (xhr.status === 500) {
					// fail_send 대응 (HttpStatus.INTERNAL_SERVER_ERROR)
					alert("메일 발송 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
				} else if (xhr.status === 400) {
					// 파라미터 누락 등 (HttpStatus.BAD_REQUEST)
					alert("입력값이 올바르지 않습니다.");
				} else {
					// 그 외 기타 통신 오류
					alert("서버 통신 오류가 발생했습니다. (에러 코드: " + xhr.status + ")");
				}
			}
		});
	}
		...
		$.ajax({
			...
			success : function(result) {
				if (result === "success") {
					...
					// 입력창 잠금 (UX 개선)
					$("#authnumber").prop("readonly", true).css(
...
<body>
	...
	<div class="container">
		...
		<header class="header">
			<h1>
				<a href="${contextPath}/member/login">UBNTIS LAB</a>
			</h1>
		...
		<div class="content">
			...
			<form action="${contextPath}/member/resetPwPage" method="post"
				name="findPwForm" class="form-box">
				...
				<div class="input-group">
					...
					<div class="input-row">
						...
						<button type="button" class="btn-small" onclick="sendAuthNum()">인증번호
							발송</button>
					</div>
				</div>
				...
				<div class="input-group">
					...
					<div class="input-row">
						...
						<button type="button" class="btn-small blue"
							onclick="checkAuthNum()">인증확인</button>
					</div>
				</div>

				<div class="btn-area">
					<button type="button" class="btn-main btn-cancel"
						onclick="history.back()">취소</button>
					<button type="button" class="btn-main btn-submit"
						onclick="findPwCheck()">다음</button>
				</div>

			</form>
		</div>

		<footer class="footer"> &copy; UBNTIS LAB Corp. All Rights
			Reserved. </footer>
